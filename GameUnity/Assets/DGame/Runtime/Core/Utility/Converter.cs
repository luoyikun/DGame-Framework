...
namespace DGame
{
    public static partial class Utility
    {
        public static class Converter
        {
            ...
            {
                // 初始化查找表
                for (int i = 0; i < HEX_LOOK_UP_TABLE.Length; i++)
                {
                    HEX_LOOK_UP_TABLE[i] = 0xFF; // 默认值，表示无效字符
                }

                // 数字 0-9
                for (int i = '0'; i <= '9'; i++)
                {
                    HEX_LOOK_UP_TABLE[i] = (byte)(i - '0');
                }

                // 大写字母 A-F
                for (int i = 'A'; i <= 'F'; i++)
                {
                    HEX_LOOK_UP_TABLE[i] = (byte)(10 + (i - 'A'));
                }
            ...
            public static float GetCentimetersFromPixels(float pixels)
            {
                if (ScreenDpi <= 0)
                {
                    throw new DGameException("需要先设置Utility.Converter.ScreenDpi");
                }
            ...
            public static void GetBytes(bool value, byte[] buffer, int startIndex)
            {
                if (buffer == null)
                {
                    throw new DGameException("Buffer is invalid.");
                }

                if (startIndex < 0 || startIndex + 1 > buffer.Length)
                {
                    throw new DGameException("Start index is invalid.");
                }
            ...
            public static byte[] GetBytes(string value, Encoding encoding)
            {
                if (value == null)
                {
                    throw new DGameException("Value is invalid.");
                }

                if (encoding == null)
                {
                    throw new DGameException("Encoding is invalid.");
                }
            ...
            private static readonly byte[] HEX_LOOK_UP_TABLE = new byte[128];
            ...
            public static Color HexToColor(string hex)
            {
                ...
                int startIndex = (hex[0] == '#') ? 1 : 0;
                int length = hex.Length - startIndex;
                ...
                byte r = (byte)((HEX_LOOK_UP_TABLE[hex[startIndex]] << 4) | HEX_LOOK_UP_TABLE[hex[startIndex + 1]]);
                byte g = (byte)((HEX_LOOK_UP_TABLE[hex[startIndex + 2]] << 4) | HEX_LOOK_UP_TABLE[hex[startIndex + 3]]);
                byte b = (byte)((HEX_LOOK_UP_TABLE[hex[startIndex + 4]] << 4) | HEX_LOOK_UP_TABLE[hex[startIndex + 5]]);
                byte a = (length == 8) ?
                    (byte)((HEX_LOOK_UP_TABLE[hex[startIndex + 6]] << 4) | HEX_LOOK_UP_TABLE[hex[startIndex + 7]]) :
                    (byte)255;
